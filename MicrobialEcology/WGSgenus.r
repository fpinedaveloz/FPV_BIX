...
p <- ggplot(g2, mapping = aes(x = Site, y = Relative_Abundance, fill = Taxonomy)) + geom_bar(position = "fill", stat = 'identity', show.legend = FALSE) +
  scale_fill_manual(values = mycolors) + 
  labs(title = 'WGS Genera by Site', caption = '* "Marginal groups" is a cumulative group that represents all genera which were beneath 1%')
...
p2 <- p1 + theme(legend.title = element_blank(), legend.position = "bottom", legend.text = element_text(size=6), legend.key.size = unit(0.25, 'cm')) + scale_y_continuous(expand = c(0, 0), labels = scales::percent)
